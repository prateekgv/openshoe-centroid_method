...
[x_h_l(1:9,1) quat_l] = init_Nav_eq(u(7:12,:));  % Subfunction located further down in the file.
...
for k = 2:N
    ...
    [x_h_r(:,k) quat_r]=Navigation_equations(x_h_r(:,k-1),u_r,quat_r); % Subfunction located further down in the file.
    [x_h_l(:,k) quat_l]=Navigation_equations(x_h_l(:,k-1),u_l,quat_l); % Subfunction located further down in the file.
...
%
%>
%> @brief Updates the navigation states with the pseudo-measuremnt.
%>
%> @param[out]  R      Rotation matrix.
%> @param[in]   ang    Euler angles [roll,pitch,heading]
%>
